...
function makeWord(lettersObject) {
  if (isEmptyObject(lettersObject)) return '';

  const letterArrays = Object.values(lettersObject);
  const flatLetterArrays = letterArrays.flat();
  const length = Math.max(...flatLetterArrays);
...
function sellTickets(queue) {
  ...
  queue.forEach((customerMoney) => {
    if (customerMoney > 25 && sellerCash <= 25) isSelled = false;
    else sellerCash += 25;
  });
...
function fromJSON(proto, json) {
  const souce = JSON.parse(json);
  const target = Object.create(proto);
  return Object.assign(target, souce);
}
...
function group(array, keySelector, valueSelector) {
  ...
  array.forEach((item) => {
    ...
    const countryList = multimap.get(key);
    const hasList = countryList && countryList.length;

    if (hasList) countryList.push(value);
